var express       = require("express");
var router        = express.Router();
var Trail         = require("../models/trail");
var Comment = require("../models/comment");
var middleware    = require("../middleware");
var geocoder = require('geocoder');
var { isLoggedIn, checkUserTrail, checkUserComment, isAdmin, isSafe } = middleware; // destructuring assignment

// Define escapeRegex function for search feature
function escapeRegex(text) {
    return text.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
};

//INDEX - show all trails
router.get("/", function(req, res){
    if (req.query.search && req.xhr){
      const regex = new RegExp(escapeRegex(req.query.search), 'gi');
    Trail.find({name: regex}, function(err, allTrails){
    if(err){
      console.log(err);
    } else {
      res.status(200).json(allTrails);
    }
});
}else{
   // Get all trails from DB
  Trail.find({}, function(err, allTrails){
    if(err){
      console.log(err);
    }else{
      if(req.xhr){
        res.json(allTrails);
      }else{
        res.render("trails/index", {trails: allTrails, page: 'trails'});
      }    }
  });
}
//CREATE - add new trails to DB
router.post("/", isLoggedIn, isSafe, function(req, res){
  //get data from form and add to trails array
  var name= req.body.name;
  var price = req.body.price;
  var image= req.body.image;
  var desc = req.body.description;
  var author = {
    id: req.user._id,
    username: req.user.username
  };
  var cost = req.body.cost;
  geocoder.geocode(req.body.location, function (err, data) {
      if (err || data.status === 'ZERO_RESULTS') {
        req.flash('error', 'Invalid address');
        return res.redirect('back');
      }
      var lat = data.results[0].geometry.location.lat;
      var lng = data.results[0].geometry.location.lng;
      var location = data.results[0].formatted_address;
      var newTrail = {name: name, image: image, description: desc, cost: cost, author:author, location: location, lat: lat, lng: lng};
      // Create a new trail and save to DB
      Trail.create(newTrail, function(err, newlyCreated){
          if(err){
              console.log(err);
          } else {
              //redirect back to trails page
              console.log(newlyCreated);
              res.redirect("/trails");
          }
      });
    });
  });

//NEW - show form to create new trail
router.get("/new", isLoggedIn, function(req, res){
  res.render("trails/new");
});

//SHOW - shows more info about one trail
router.get("/:id", function(req, res){
  //find the trail with provided ID
  Trail.findById(req.params.id).populate("comments").exec(function(err, foundTrail){
    if(err || !foundTrail){
      console.log(err);
      req.flash('error', "Sorry, that trail does not exist!");
      return res.redirect('/trails');
    }  console.log(foundTrail);
      //render show template with that trail
      res.render("trails/show", {trail: foundTrail});
  });
});

//Edit trail route
router.get("/:id/edit", isLoggedIn, checkUserTrail, function(req, res) {
    res.render("trails/edit", {trail: req.trail});
  });

//Update trail route
router.put("/:id", isSafe, function(req, res){
  geocoder.geocode(req.body.location, function (err, data) {
    var lat = data.results[0].geometry.location.lat;
    var lng = data.results[0].geometry.location.lng;
    var location = data.results[0].formatted_address;
    var newData = {name: req.body.name, image: req.body.image, description: req.body.description, cost: req.body.cost, location: location, lat: lat, lng: lng};
  //find and update the correct trail
  Trail.findByIdAndUpdate(req.params.id, {$set: newData}, function(err, trail){
    if(err){
      req.flash("error", err.message);
      res.redirect("back");
    } else{
      //redirect to show page
      req.flash("success", "Successfully Updated!");
      res.redirect("/trails/" + req.params._id);
    }
  });
});
});

//Destroy Trail Route
router.delete("/:id", isLoggedIn, checkUserTrail, function(req, res){
  Comment.remove({
    _id:{
      $in: req.trail.comments
    }
  }, function (err){
    if(err){
      req.flash('error', err.message);
      res.redirect("/");
    } else {
    req.trail.remove(function(err){
      if(err){
        req.flash('error', "Trail Deleted!");
          res.redirect("/trails");
        });
    }
  })
});

module.exports = router;
